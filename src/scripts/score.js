...
//           `Ook wanneer er niet direct een probleem is, is het goed om regelmatig en blijvend kritisch te kijken naar je organisatie. 
//           Om de juiste richting voor de toekomst te kiezen of om de gekozen richting te checken en waar nodig bij te sturen. 
//           Vernieuw sowieso je middellange termijnplanning om de 2-3 jaren. Dat houd je alert en fris. 
//           En spar met anderen (vooral ook buiten de branche, of een goede mentor) om scherp te blijven. 
//           De snelheid waarmee bijv. de Coronacrisis recent de wereld veranderde is daar een goed voorbeeld van.`,
//         ],
//         Conclusie4: [
//           // Conclusie 4.0 | als vraag 1 > 2 
//           `Als je hulp nodig hebt om betrokken kritisch en ook objectief met je mee te kijken, 
//           zodat je de juiste beslissingen kan nemen om succesvol te blijven ondernemen, 
//           zijn wij er voor je. Dat is ons vak, en we zijn er goed in. 
//           Neem daarom contact op voor een -online- kennismakingsgesprek en deel je ideëen eens met ons.`,
//         ],
//       }; //end return
//     }, //end data
//   }; // end export
...
function resultaatScore(opmerkingen, vragen){
  ...
  const resultaat = {
    ...
    conclusie3 : {opmerking : "", score : ""}, 
    conclusie4 : {opmerking : "", score : ""}, 
  }
  ...
  // bereken totaal
  const totaal = convertStrToNum(opmerkingen.valueV1[0]) + convertStrToNum(opmerkingen.valueV1[1]) + convertStrToNum(opmerkingen.valueV1[2]) + convertStrToNum(opmerkingen.valueV1[3]) + convertStrToNum(opmerkingen.valueV1[4]);
  resultaat.vraag1.totaal = totaal;

  if( vraag1.waarde <2 ){
    resultaat.vraag1.opmerking = opmerkingen.OpmerkingenV1[0].valueV1[0];
    resultaat.vraag1.berekening = (convertStrToNum(opmerkingen.valueV1[0]) / totaal) * 100;
  }
  else if(vraag1.waarde >=2 && vraag1.waarde < 10 ){
    resultaat.vraag1.opmerking = opmerkingen.OpmerkingenV1[1].valueV1[1];
    resultaat.vraag1.berekening = (convertStrToNum(opmerkingen.valueV1[1]) / totaal) * 100;
  }
  else if(vraag1.waarde >=10 && vraag1.waarde < 50 ){
    resultaat.vraag1.opmerking = opmerkingen.OpmerkingenV1[1].valueV1[2];
    resultaat.vraag1.berekening = (convertStrToNum(opmerkingen.valueV1[2]) / totaal) * 100;
  }
  else if(vraag1.waarde >=50 && vraag1.waarde < 100 ){
    resultaat.vraag1.opmerking = opmerkingen.OpmerkingenV1[1].valueV1[3];
    resultaat.vraag1.berekening = (convertStrToNum(opmerkingen.valueV1[3]) / totaal) * 100;
  }
  else if(vraag1.waarde >=100 && vraag1.waarde < 250 ){
    resultaat.vraag1.opmerking = opmerkingen.OpmerkingenV1[1].valueV1[4];
    resultaat.vraag1.berekening = (convertStrToNum(opmerkingen.valueV1[4]) / totaal) * 100;
  }
